...
func main() {
	...
	for _, frInt := range []int{8000, 11000, 16000, 44100, 48000} {
		for _, toInt := range []int{8000, 16000} {
			if toInt == frInt {
				continue
			}
			...
			f.Func().Params(Id("rsm").Op("*").Id(sName)).Id("Resample").Params(Id("inp").Index().Int16()).Error().Block(
				Qual("resampler/internal/resample/resamplerl", rFuncName).Call(Id("inp"), Id("rsm").Dot("resampled")),
				Return(Nil()),
			)
			...
			f.Func().Id(tFuncName).Params(Id("t").Op("*").Qual("testing", "T")).Null().Block(
				Var().Id("tObj").Qual(tUtilsS, "TestObj").Op("=").Qual(tUtilsS, "TestObj").Values().Dot("New").Call(
					Qual(tUtilsS, "SinWave").Values().Dot("New").Call(Lit(0), Lit(50), Lit(frInt), Lit(toInt)),
	...
	err := f.Save(*outF)
	if err != nil {
		log.Fatal("failed to save generated file")
	}
}
